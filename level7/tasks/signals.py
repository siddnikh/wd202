...
def update_history(sender, instance, **kwargs):
    h = History.objects.filter(task = instance).last()
    ...
    # When a task's status is updated
        ...
        History.objects.create(task = instance, old_status = history_queryset[history_queryset.count() - 1].new_status, new_status = instance.status)