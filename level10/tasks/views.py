...
from .utils import AuthenticationManager, TaskCreateForm, PassRequestToFormViewMixin, UserLoginForm, UserSignUpForm, TaskCascadeMixin, ReportForm
...
class ReportConfigureView(PassRequestToFormViewMixin, CreateView, LoginRequiredMixin):
    template_name = 'tasks/user_configure.html'
    form_class = ReportForm
    success_url = '/tasks/'

    def form_valid(self, form):
        r = Report.objects.filter(user = self.request.user).first()
        if r is not None:
            r.update(time=form.cleaned_data['time'])
